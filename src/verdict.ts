...
export function computeVerdict(
  ...
  for (const r of interrupted)
    reasons.push(`${r.id}: ${r.status} (run interrupted)`);
  const requiredUnavailable = results.filter(
    (r) => r.required && r.status === "unavailable",
  );
  for (const r of requiredUnavailable)
    reasons.push(`${r.id}: required check unavailable`);
  const optionalUnavailable = results.filter(
    (r) => !r.required && r.status === "unavailable",
  );
  for (const r of optionalUnavailable)
    reasons.push(`${r.id}: optional check unavailable`);
